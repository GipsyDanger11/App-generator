...
export function GithubExport({ appId, defaultName }: { appId: string; defaultName: string }) {
  ...
  return (
    <div className="max-w-2xl mx-auto card p-6">
      <div className="flex items-center gap-2 mb-2"><GithubIcon className="h-5 w-5" /><h2 className="font-semibold">Export to GitHub</h2></div>
      ...
      <div className="mt-4 space-y-3">
        ...
        <button onClick={exportNow} disabled={busy} className="btn">
          {busy ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : <GithubIcon className="h-4 w-4 mr-1" />}
